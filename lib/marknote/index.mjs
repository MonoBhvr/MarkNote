...
function block (state, startLine, endLine, silent) {
  ...
  if (attr === '>' || attr === 'fold') {
    ...
    token.content = `<details class="marknote-fold"><summary>${state.md.renderInline(summary, state.env)}</summary>${state.md.render(body, state.env)}</details>\n`
  } else if (attr === 'image') {
    const { value, label } = splitEmbed(parsed.content)
    token.content = `<figure class="marknote-embed marknote-embed-image"><img src="${escapeHtml(value)}" alt="${escapeHtml(label)}">${label ? `<figcaption>${escapeHtml(label)}</figcaption>` : ''}</figure>\n`
  } else if (attr === 'web') {
    const { value, label } = splitEmbed(parsed.content)
    token.content = `<figure class="marknote-embed marknote-embed-web"><iframe sandbox="allow-same-origin allow-scripts" src="${escapeHtml(value)}" title="${escapeHtml(label || value)}"></iframe>${label ? `<figcaption>${escapeHtml(label)}</figcaption>` : ''}</figure>\n`
  } else if (attr === 'html') {
    token.content = `<iframe class="marknote-embed marknote-embed-html" sandbox srcdoc="${escapeHtml(parsed.content)}"></iframe>\n`
  } else if (attr === 'card') {
    const { value, label } = splitEmbed(parsed.content)
    token.content = `<a class="marknote-card" href="${escapeHtml(value)}">${escapeHtml(label || value)}</a>\n`
  } else {
    token.content = `<div class="marknote-block">${state.md.render(parsed.content, state.env)}</div>\n`
  }
