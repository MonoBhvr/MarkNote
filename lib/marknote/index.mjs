  ...
  while (pos < state.src.length) {
    ...
    if (state.src[pos] === ']' && state.src[pos + 1] === ']') {
      ...
      if (depth === 0) {
        ...
        return { attr, content, nextLine: line }
      }
      pos++
    }
    pos++
...
function parseBlockAttrs (attr) {
  const tokens = attr.split(/\s+/).map(token => token.trim()).filter(Boolean)
  const embeds = tokens.filter(token => EMBED_ATTRS.has(token))
...
function blockColorStyle (color) {
  if (!color) return ''
  if (HEX_COLOR_RE.test(color)) {
    const value = color.startsWith('#') ? color : `#${color}`
    return ` style="--marknote-block-color: ${escapeHtml(value)}"`
  }
  return ''
}
...
function decorateBlock (content, attrs) {
  ...
  return `<div class="${blockClass(attrs)}"${blockColorStyle(attrs.color)}>${marker}<div class="marknote-block-content">${content}</div></div>\n`
}

function renderEmbed (embed, content) {
  if (embed === 'image') {
    const { value, label } = splitEmbed(content)
    return `<figure class="marknote-embed marknote-embed-image"><img src="${escapeHtml(value)}" alt="${escapeHtml(label)}">${label ? `<figcaption>${escapeHtml(label)}</figcaption>` : ''}</figure>\n`
  }
  if (embed === 'web') {
    const { value, label } = splitEmbed(content)
    return `<figure class="marknote-embed marknote-embed-web"><iframe sandbox="allow-same-origin allow-scripts" src="${escapeHtml(value)}" title="${escapeHtml(label || value)}"></iframe>${label ? `<figcaption>${escapeHtml(label)}</figcaption>` : ''}</figure>\n`
  }
  if (embed === 'html') return `<iframe class="marknote-embed marknote-embed-html" sandbox srcdoc="${escapeHtml(content)}"></iframe>\n`
  if (embed === 'card') {
    const { value, label } = splitEmbed(content)
    return `<a class="marknote-card" href="${escapeHtml(value)}">${escapeHtml(label || value)}</a>\n`
  }
  return ''
}

function renderBlockBody (state, attrs, content) {
  if (attrs.embed) return renderEmbed(attrs.embed, content)
...
function block (state, startLine, endLine, silent) {
  ...
  const attrs = parseBlockAttrs(parsed.attr)
  ...
  if (attrs.fold) {
    ...
    const marker = attrs.type ? `<span class="marknote-block-marker" aria-hidden="true">${TYPE_LABELS[attrs.type]}</span>` : ''
    token.content = `<details class="${foldClass(attrs)}"${attrs.foldOpen ? ' open' : ''}${blockColorStyle(attrs.color)}><summary>${marker}<span>${state.md.renderInline(summary, state.env)}</span></summary><div class="marknote-block-content">${bodyContent}</div></details>\n`
  } else if (attrs.embed && !attrs.type && !attrs.color) {
    token.content = renderEmbed(attrs.embed, parsed.content)
  } else {
